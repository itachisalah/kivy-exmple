"""
Write the code for Reynolds’s wandering behavior. Use polar coordinates to
calculate the vehicle’s target along a circular path.

TODO: Along a circular path???
"""
...
class Car(Widget):
    ...
    def setGoal(self):
        # Pick a point at distance away from current location
...
class Universe(Widget):
    ...
    def add_child(self, num):
        for _ in range(num):
            c = Car()
            self.add_widget(c)
            Clock.schedule_interval(c.update, .05)
